<template>
  <div class="schedule-appointment">
    ...
    <div class="card">
      <div v-if="currentUser === undefined" class="card-info">
      ...
      <div v-else class="card-info">
        ...
        />
        <h1>Call with {{ currentUser.name }}</h1>
        <div class="card-info-details">
          <h3>Duration: {{ currentUser.duration }} minutes</h3>
          <h3>Cost: ${{ currentUser.cost }}.00</h3>
        </div>
...
export default {
  ...
  computed: {
    ...
    dateToTimeSlots() {
      let dateToTimeSlots = {};
      if (this.currentUser) {
        this.currentUser.availability.forEach(availabilitySlot => {
          const {
            year,
            month,
            day,
    ...
    timeSlots() {
      if (Object.keys(this.dateToTimeSlots).length && this.currentDate) {
        ...
        let numSlots = Math.floor(minutes / this.currentUser.duration);
        let slots = [startTime];
        for (let i = 0; i < numSlots - 1; i++) {
          slots.push(
            new Date(slots[i].getTime() + this.currentUser.duration * 60000)
          );
        }
        return slots;
        // return [this.dateToTimeSlots[this.currentDate.date]];
      } else {
        return [];
      }
    }
  ...
  watch: {
    ...
    currentUser(newUser, oldUser) {
      ...
      if (oldUser) {
        for (let availabilityDate of oldUser.availability) {
          this.dates[availabilityDate.day - 1].option = false;
        }
      }
      for (let availabilityDate of newUser.availability) {
